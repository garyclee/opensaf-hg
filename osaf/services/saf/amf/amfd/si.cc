...
SaAisErrorT AVD_SI::si_swap_validate()
{
	...
	if (saAmfSIAdminState != SA_AMF_ADMIN_UNLOCKED) {
		LOG_NO("%s SWAP failed - wrong admin state=%u", name.value,
				saAmfSIAdminState);
		rc = SA_AIS_ERR_BAD_OPERATION;
		goto done;
	}
	...
	if (avd_cb->init_state != AVD_APP_STATE) {
		LOG_NO("%s SWAP failed - not in app state (%u)", name.value,
				avd_cb->init_state);
	...
	if (sg_of_si->sg_fsm_state != AVD_SG_FSM_STABLE) {
		LOG_NO("%s SWAP failed - SG not stable (%u)", name.value,
	...
	if (is_assigned() == false) {
		LOG_NO("%s SWAP failed - no assignments to swap", name.value);
		rc = SA_AIS_ERR_BAD_OPERATION;
		goto done;
	}
	...
	if (sg_of_si->is_middleware() == true) {
		...
		if (avd_cb->stby_sync_state == AVD_STBY_OUT_OF_SYNC) {
			LOG_NO("%s SWAP failed - Cold sync in progress", name.value);
			rc = SA_AIS_ERR_TRY_AGAIN;
			goto done;
		}
	}
