...
void verLogFileRotate(void)
{
	...
	uint32_t fileSize = 0;
	char logRecord[1000];
	uint32_t logMaxSize = 1024, fixedLogRecSize = 0;
	...
	/* Restore the attributes to previous value */
	sprintf(command,"immcfg safLgStrCfg=saLogSystem,safApp=safLogService"
			" -a saLogStreamMaxLogFileSize=%d"
			" -a saLogStreamFixedLogRecordSize=%d 2> /dev/null",
			logMaxSize, fixedLogRecSize);
	rc = system(command);
}
...
__attribute__ ((constructor)) static void saOiOperations_constructor(void)
{
	...
	test_case_add(6, verLogFileRotate, "Verify that log file is not rotated if file size doesn't reach max file size (ticket1439)");
}
